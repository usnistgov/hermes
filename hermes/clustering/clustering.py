...
class Cluster(IDA):
    ...
    measurement_similarity: np.ndarray  # CQ suggestion: call it similarity
...
class ContiguousCluster(Cluster):
    ...
    def get_local_membership_prob(
        self, graph: nx.Graph, cluster_labels: np.ndarray, v: int = 1
    ):  # CQ can graph only be the graph generated by the fn above?
...
class ContiguousFixedKClustering(ContiguousCluster):
    ...
    def __post_init__(self):
        ...
        )  # CQ is it similarity or distance?
