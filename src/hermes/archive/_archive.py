...
class CombiMappingModels(JSONizer):
    ...
    def write_loopdata_file(self, Loop_index: int):
        ...
        results_dictionary["probabilities"] = str(
            self.cluster_method.probabilities.tolist()
        )

        results_dictionary["kernel"] = self.classification_method.model.kernel.name
        results_dictionary["k_lengthscale"] = str(
            self.classification_method.model.kernel.lengthscales.numpy()
        )
        results_dictionary["k_variance"] = str(
            self.classification_method.model.kernel.variance.numpy()
        )

        results_dictionary["mean"] = str(
            self.classification_method.mean.numpy().tolist()
        )
